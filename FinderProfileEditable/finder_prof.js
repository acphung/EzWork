...
function showedit() {
	...
	document.getElementById("namebox").innerHTML = boxval;
}
...
function showedit2() {
	...
	document.getElementById("loc").innerHTML = box1val;
}
...
function showedit3() {
	...
	document.getElementById("abt").innerHTML = box2val;
}
...
function showedit4() {
	...
	document.getElementById("phonebox").innerHTML = box3val;
}
...
firebase.auth().onAuthStateChanged(function(u){
    ...
    if(u) {
		firebase.database().ref('users/' + user.email.replace('.','(')).once('value', function(snapshot) {
			...
			var name = snapshot.val().name || "None";
			var lloc = snapshot.val().loc || "None";
			var phone = snapshot.val().phone || "None";
			var about = snapshot.val().about || "None";
			document.querySelector("#namebox").innerHTML = name;
			document.querySelector("#phonebox").innerHTML = phone;
			document.querySelector("#loc").innerHTML = lloc;
			document.querySelector("#abt").innerHTML = about;
		...
		firebase.storage().ref('users/'+ user.email.replace('.','(') + '/resume.pdf').getDownloadURL().then(
			function(URL){
				...
				document.querySelector("#res").href = "#";
			});
    }
});
